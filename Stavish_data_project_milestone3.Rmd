...
max_public_transportation <- public_transportation %>%
  clean_names() %>%
  rename(public_trans = "x2020_data") %>%
  slice_max(public_trans, n=5)
...
#Map on public transportation use
#Note, I want to convert this to a leaflet, but I did a ton of research and it is slightly confusing on how to do that. For now, I was able to create this chloropleth map using ggplot.
```{r}
...
```
# Map on amount of public transportation stops in each neighborhood
```{r}
...
stops_popup <- paste0("<strong>Neighborhood: </strong>", 
                      geo_stops$community, 
                      "<br><strong>Percent that Uses Public Transportation </strong>", 
                      geo_stops$public_transportation_pct)
